 ...
 implements ActionListener, DocumentListener
{
  ...
  void checkOutputOptions()
  {
    ...
    defaultButton.setToolTipText(
     defaultButton.isEnabled() ? "start generation process (Return)" :
     someFile | someViewer ? "don't mix viewer and file output" :
     "choose some output options and press Return");
  }
  ...
  int addViewers
   (String dimName, Vector viewersDim,
    GenericButtonGroup buttonGroup, JComponent component)
  {
	...
	if (! ViewerFactory.checkAvailability(viewerName, dimension)) continue;
	...
	if (i == 1) viewersXDGroup.add(viewerButton);
      }
    }
    return n;
  }
  ...
  void addSelectedViewers 
   (Vector viewers, GenericButtonGroup buttonGroup, int dimension)
  {
  ...
  void addWriter
   (Vector writers, FileFormatBox format, int dimension)
  {
...
class onActionClicker implements ActionListener
{
  ...
  public void actionPerformed(ActionEvent e)
  {
    ...
    if (source.isSelected()) {
      if (buttonTrue != null) {
        buttonTrue.doClick();
        if (! buttonTrue.isVisible()) source.transferFocus();
      }
    } else {
      if (buttonFalse != null) {
        buttonFalse.doClick();
 ...
 implements ChangeListener, KeyListener, MouseListener
{
  ...
  public Min1ButtonGroup
   (String id, boolean active, AbstractButton deactivateButton)
  {
    this.id = id;
    this.active = active;
  ...
  public void add(AbstractButton button)
  {
    ...
    if (button.isSelected()) selections.put(button, this);
  }
  ...
  public void remove(AbstractButton button)
  {
    ...
    if (button.isSelected()) selections.remove(button);
  }
...
class SyncButtonGroup extends AbstractButtonGroup
{
  ...
  public void itemStateChanged(ItemEvent e)
  {
    ...
    if (change != ItemEvent.SELECTED && change != ItemEvent.DESELECTED) return;
    AbstractButton button = (AbstractButton) e.getSource();
    boolean selected = button.isSelected();
    ...
    while (btns.hasMoreElements())
    {
      ...
      if (otherButton.isSelected() == selected) continue;
      otherButton.setSelected(selected);
