 ...
 ActionListener, FocusListener, GeneratorListener, EmbedThreadListener
{
  ...
  static final int graphNoFireInterval
   = CaGe.getCaGePropertyAsInt("CaGe.GraphNoFireInterval.Foreground", 0);
  static final int graphNoFirePeriod
   = CaGe.getCaGePropertyAsInt("CaGe.GraphNoFirePeriod.Foreground", 1000);
  ...
  public ResultPanel
   (CaGePipe gen, GeneratorInfo info,
...
//    advanceDist.setMajorAdjust(1);
  ...
  void advanceBy(int n)
  {
    if (generator.isRunning()) {
      setStatus("advancing to graph "
       + (generator.getGraphNo() + n) + " ...", ADVANCE_LEVEL);
      previousFocusOwner = advance1Button;
      try {
	...
	if (previousFocusOwner != null)
      ...
      if (debug) System.err.println("advanceBy(" + n + ") completed.");
    } else {
  ...
  public void start()
  {
    try {
      ...
      advanceBy(1);
      if (debug) System.err.println("start completed.");
    } catch (Exception ex) {
      clearStatus(RUN_LEVEL, false);
  ...
  public void stop()
  {
    ...
    stopAll(viewers);
    stopAll(writers);
    if (saveAdjWriter != null) saveAdjWriter.stop();
    if (save2DWriter != null) save2DWriter.stop();
    if (save3DWriter != null) save3DWriter.stop();
    saveAdjWriter = save2DWriter = save3DWriter = null;
    CaGe.foldnetThread().removePropertyChangeListener(this);
    clearStatus(RUN_LEVEL);
    if (debug) System.err.println("stopped.");
  }
  ...
  void stopAll(CaGeOutlet[] outlet)
  {
    ...
    for (int i = 0; i < outlet.length; ++i) outlet[i].stop();
  }
  ...
  private void updateView()
  {
    ...
    if (viewGraphNo.hasFocus()) viewGraphNo.selectAll();
  ...
  void outputResult
   (CaGeOutlet[] outlet, CaGeResult result, boolean has2D, boolean has3D)
  {
    if (outlet == null) return;
  ...
  void changeFlow()
  {
    ...
    setFlowing(flowing);
    if (debug) System.err.println("setFlowing completed.");
  }
  ...
  CaGeWriter saveResult
   (CaGeResult result, CaGeWriter saveWriter,
    AbstractButton saveButton, String variety, String dialogTitle)
  {
    ...
    if (saveWriter == null) return null;
    ...
    if (saveWriter.wasIOException()) {
      ...
      if (previousFocusOwner != null) previousFocusOwner.requestFocus();
    }
    return saveWriter;
  }
  ...
  */
  ...
  {
    if (debug) System.err.println("property changed: " + e.getPropertyName());
  ...
  */
  void handlePropertyChange(PropertyChangeEvent e)
  {
	...
	if (left > 0) {
	  setStatus(left + " folding net" + (left == 1 ? "" : "s")
	   + " left to make.", FOLDNET_LEVEL);
	  clearStatus(FOLDNET_LEVEL, false);
	} else {
  ...
  void graphNoChanged(int graphNo)
  {
    ...
    if (debug) System.err.println("graphNo changed: " + graphNo + ", flowing=" + flowing);
    if (flowing) return;
    if (stopping) return;
  ...
  {
    if (debug) System.err.println("flowing changed: " + flowing);
  ...
  {
    if (debug) System.err.println("running changed: " + running);
    this.running = running;
    if (running) return;
    if (stopListener != null) stopListener.actionPerformed(new ActionEvent(this, 0, null));
    if (timer != null) timer.stop();
    viewGraphNo.requestFocus();
  ...
  void embeddingMade(CaGeResult result)
  {
    ...
    updateView();
    if (! (useViewers || stopping)) advanceBy(1);
  }
  ...
  public void showEmbeddingException
   (final Exception ex, final String context, final String diagnosticOutput)
  {
  ...
  void showException
   (final Exception ex, final String context,
  ...
  public void embeddingModified(CaGeViewer modifyingViewer, CaGeResult result)
  {
    ...
    EmbeddableGraph graph = result.graph;
    boolean modifiedOk = modifiedDimension == 2 ?
     graph.has2DCoordinates () : graph.has3DCoordinates();
    if (! modifiedOk) return;
    AbstractButton saveButton = modifiedDimension == 2 ?
     save2DButton : save3DButton;
...
class GraphNoLabel extends JLabel
{
  ...
  public GraphNoLabel(String text)
  {
    ...
    minimumSize.width
     = metrics.stringWidth(Systoolbox.multiply("0", CaGe.graphNoDigits)) + 4;
