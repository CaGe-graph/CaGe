 ...
 implements CaGeRunner, PropertyChangeListener, EmbedThreadListener
{
  ...
  static final int graphNoFireInterval
   = CaGe.getCaGePropertyAsInt("CaGe.GraphNoFireInterval.Background", 10);
  static final int graphNoFirePeriod
   = CaGe.getCaGePropertyAsInt("CaGe.GraphNoFirePeriod.Background", 10000);
  ...
  public BackgroundRunner
   (CaGePipe generator, GeneratorInfo generatorInfo,
  ...
  {
    if (debug) System.err.println("halted: " + halted);
    this.halted = halted;
  }
  ...
  {
    if (debug) new Exception("queueing property change: " + e.getPropertyName() + " = " + e.getNewValue() + " (old value: " + e.getOldValue() + ")").fillInStackTrace().printStackTrace();
    queue.put(e);
  }

  void handlePropertyChange(PropertyChangeEvent e)
  {
    if (debug) System.err.println("handling property change: " + e.getPropertyName() + " = " + e.getNewValue() + " (old value: " + e.getOldValue() + ")");
	...
	break;
      default:
	if (debug) System.err.println("unimplemented property change: " + e.getPropertyName());
	break;
    }
  }
  ...
  {
    if (debug) System.err.println("graphNo: " + graphNo);
    if (graphNo <= 0) return;
    if (generatorFlowing) return;
    if (embedThread == null) return;
    try {
  ...
  void flowingChanged(boolean flowing)
  {
    ...
    if (debug) System.err.println("flowing: " + generatorFlowing);
  }

  void runningChanged(boolean running)
  {
    generatorRunning = running;
    if (debug) System.err.println("running: " + generatorRunning);
    if (generatorRunning) {
      fireRunningChanged();
  ...
  void embeddingMade(CaGeResult result, boolean success)
  {
	...
	fireGraphNoChanged();
      ...
      try {
	if (generator.isRunning()) generator.yieldAndAdvanceBy(1);
      } catch (Exception ex) {
	generator.fireExceptionOccurred(ex);
	end();
      }
  ...
  void finish()
  {
    ...
    if (generatorRunning) {
      if (debug) System.err.println("announcing crash");
  ...
  {
    if (debug) System.err.println("announcing running change: " + generatorRunning);
    firePropertyChange(
     new PropertyChangeEvent(
