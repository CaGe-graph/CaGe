 ...
 implements PropertyChangeListener, Runnable, ActionListener
{
  ...
  public void adjustDisplayedRunners(int adjust)
  {
    ...
    pack();
    if (exitPhase == 0) setVisible(showingAny);
  }
  ...
  public void run()
  {
    ...
    if (! showingRunners) setVisible(true);
    checkForExit();
  }
 ...
 implements PropertyChangeListener, ActionListener
{
    ...
    BackgroundWindow window, int index, JPanel panel)
  {
    ...
    if (running) window.adjustActiveRunners(+1);
    infoText = new StringBuffer();
    infoText.append("generator:\t "
     + generatorInfo.getGeneratorName() + "\n");
    if (CaGe.expertMode) {
      infoText.append("  command:\t "
       + Systoolbox.makeCmdLine(generatorInfo.getGenerator()) + "\n");
    }
    infoText.append("\noutput:\n");
