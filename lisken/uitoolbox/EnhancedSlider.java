...
public class EnhancedSlider extends JPanel implements FocusListener, Serializable
{
  ...
  { slider.setModel(newModel); valueLabel.calculateSize(); }
  public int getValue()
  ...
  { slider.setMinimum(minimum); valueLabel.calculateSize(); }
  public int getMaximum()
  { return slider.getMaximum(); }
  public void setMaximum(int maximum)
  { slider.setMaximum(maximum); valueLabel.calculateSize(); }
  ...
  {
    if (debug) System.err.println("  slider focus");
    slider.requestFocus();
  }
...
class EnhancedSliderUI extends javax.swing.plaf.basic.BasicSliderUI
{
  ...
  public Rectangle getTrackRectangle() { return trackRect; }
  public Rectangle getThumbRectangle() { return thumbRect; }
  ...
  protected void paintMinorTickForHorizSlider
   (Graphics g, Rectangle tickBounds, int x)
  {
  ...
  protected void paintMinorTickForVertSlider
   (Graphics g, Rectangle tickBounds, int y)
  {
    if (enhancedSlider.getPaintMinorTicks()) {
  ...
  {
    return enhancedSlider.getSnapToTicks() ?
     enhancedSlider.getMinorTickSpacing() : 1;
  }
...
//    this.setOpaque(true);
  ...
  {
    if (slider == null) {
      if (EnhancedSlider.debug) System.err.println("  new place as requested: " + x + ", " + y);
  ...
  public void adjustBounds(int newWidth, int newHeight)
  {
    ...
    {
      case SwingConstants.HORIZONTAL:
        if (EnhancedSlider.debug) System.err.println("  new width: " + newWidth + ", th = " + rect.x + ", " + rect.width);
        if (EnhancedSlider.debug) System.err.println("  new place: " + (rect.x + (rect.width - newWidth) / 2) + ", " + getLocation().y);
        super.setBounds(rect.x + (rect.width - newWidth) / 2, getLocation().y, newWidth, newHeight);
        break;
      case SwingConstants.VERTICAL:
        if (EnhancedSlider.debug) System.err.println("  new height: " + newHeight + ", th = " + rect.y + ", " + rect.height);
        if (EnhancedSlider.debug) System.err.println("  new place: " + getLocation().x + ", " + (rect.y + (rect.height - newHeight) / 2));
