...
public class Systoolbox
{
  ...
  public static String replace
   (String string, String oldString, String newString)
  {
    int o = oldString.length();
    if (o == 0) return string;
    ...
    while (i < s)
    {
      if (result.charAt(i++) == oldString.charAt(j++)) {
        if (j == o) {
	  result = result.substring(0, i - o)
	   + newString + result.substring(i);
	  s += d;
	  i += d;
	  j = 0;
	}
  ...
  public static int firstIndexOf
   (String string, String[] substringArray, int fromIndex)
  {
    ...
    for (int index = fromIndex; index < length; ++index)
    {
	  ...
	  if (index + subLength > length) continue;
  ...
  public static BufferedFDOutputStream createOutputStream
   (String cmdOrFilename)
   throws Exception
  {
  ...
  public static BufferedFDOutputStream createOutputStream
   (String cmdOrFilename, String rootDir)
   throws Exception
  {
  ...
  public static BufferedFDOutputStream createOutputStream
   (String cmdOrFilename, String rootDir, boolean append)
   throws Exception
  {
